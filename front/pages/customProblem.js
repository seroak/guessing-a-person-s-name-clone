...
const customProblem = () => {
  ...
  // 문제 가져오는 initialState useSelector
  const { problemList } = useSelector((state) => state.problem);
  // 가져온 initialState 저장하는 useState
  ...
  const checkValue = () => {
    ...
    if (trimmedValue !== "") {
      if (
        ...
        problem[score].Name2 === trimmedValue ||
  ...
  const gamestart = (event) => {
    ...
    setProblem(shuffleArray(newProblem));
    setProblemTime(selectedOption);
    setCountStart(true);
  };
  useEffect(() => {}, [problem]);
              ...
              isGameStart ? (
                finish ? (
                  ...
                  <img
                    ...
                    src={
                      problem.length > 0
                        ? `http://localhost:3065/${problem[score]?.Image}`
                        : ""
                    }
                  />
                )
